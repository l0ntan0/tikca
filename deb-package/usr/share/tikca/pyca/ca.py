...
# PZ it's okay for it to stay here. But it doesn't have to.
...
def ingest(tracks, recording_dir, recording_id, workflow_def,
    ...
    print("MEDIAPACKAGE", mediapackage)
    ...
    if os.path.isfile('%s/episode.xml' % recording_dir):
        ...
        print(fields)
        mediapackage = http_request('/ingest/addDCCatalog', list(fields))
    ...
    for (flavor, track) in tracks:
        print("adding track", flavor, track)
